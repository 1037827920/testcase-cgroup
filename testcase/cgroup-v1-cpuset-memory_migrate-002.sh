#!/bin/bash
###############################################################################
# @用例ID: 20240811-222550-389275834
# @用例名称: cgroup-v1-cpuset-memory_migrate-002
# @用例级别: 2
# @用例标签: cgroup-v cpuset memory_migrate
# @用例类型: 测试memory_migrate接口文件, 测试不启用内存迁移
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v1-cpuset-memory_migrate-002"
...
do_test() {
    ...
    # @测试步骤:7: 修改cpuset.mems
    echo "$second_online_mem" >"$CGROUP_TOPDIR"/cpuset/$CGROUP/cpuset.mems
    sleep 3

    # @测试步骤:8: 检查内存页是否迁移，使用numastat命令
    memory_usage_in_numa0=$(numastat -p $task_pid | awk 'END{print $2}')
    memory_usage_in_numa1=$(numastat -p $task_pid | awk 'END{print $3}')

    # @预期结果:1: 进程在numa_node0上有内存使用，在numa_node1也有内存使用
    assert_true [ "$(echo "$memory_usage_in_numa0 > 0" | bc)" -eq 1 ] && [ "$(echo "$memory_usage_in_numa1 > 0" | bc)" -eq 1 ]
