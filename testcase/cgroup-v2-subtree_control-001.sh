#!/bin/bash
###############################################################################
# @用例ID: 20240810-000034-129698018
# @用例名称: cgroup-v2-subtree_control-001
# @用例级别: 1
# @用例标签: cgroup-v2 subtree_control
# @用例类型: 测试cgroup.subtree_control接口文件，不能够在domain cgroup中加入进程在启用了subtree_control的cgroup中
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
