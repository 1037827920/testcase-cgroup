#!/bin/bash
###############################################################################
# @用例ID: 20240809-171019-081310498
# @用例名称: cgroup-v2-hugetlb-max-004
# @用例级别: 2
# @用例标签: cgroup v2 hugetlb max
# @用例类型：测试hugetlb.1GB.max接口文件， 不超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-hugetlb-max-004"
TOLERANCE=0.3
...
do_test() {
    ...
    # @测试步骤:6: 设置CGROUP的hugetlb.1GB.max为1GB
    echo 1024M >"$CGROUP_TOPDIR"/$CGROUP/hugetlb.1GB.max
    ...
    # @测试步骤:8: 获取hugetlb.1GB.current
    total_hugepage_current=0
    for _ in $(seq 1 5); do
        current=$(cat "$CGROUP_TOPDIR"/$CGROUP/hugetlb.1GB.current)
