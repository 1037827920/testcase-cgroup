#!/bin/bash
###############################################################################
# @用例ID: 20240804-155658-686144114
# @用例名称: cgroup-v2-cpu-weight-001
# @用例级别: 1
# @用例标签: cgroup-v2 cpu weight
# @用例类型: 测试cpu.weight接口文件
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-cpu-weight-001"
...
do_test() {
    ...
    echo 512 >"$CGROUP_TOPDIR"/$CHILD1/cpu.weight
    ...
    usage_ratio=$(echo "scale=2; $cpu_usage_usec1 / $cpu_usage_usec2 + 0.5" | bc)
    usage_ratio=$(printf "%.0f" "$usage_ratio")
    msg "usage_ratio: $usage_ratio"
    assert_true [ "$usage_ratio" -eq 4 ]
