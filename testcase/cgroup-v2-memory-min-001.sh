#!/bin/bash
###############################################################################
# @用例ID: 20240806-100136-390417533
# @用例名称: cgroup-v2-memory-min-001
# @用例级别: 1
# @用例标签: cgroup-v2 memory min
# @用例类型: 测试memoruy.min接口文件，parent1/2的父cgroup设置的内存上限是200MB, parent2分配的内存为143MB, 不会占用parent1的memory.min保护的内存50MB
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-memory-min-001"
...
do_test() {
    ...
    # @测试步骤:2: 设置memory.max和memory.min
    echo 200M >"$CGROUP_TOPDIR"/$CGROUP/memory.max
    echo 50M >"$CGROUP_TOPDIR"/$PARENT1/memory.min
    echo 75M >"$CGROUP_TOPDIR"/$CHILD0/memory.min
    echo 25M >"$CGROUP_TOPDIR"/$CHILD1/memory.min
    echo 0M >"$CGROUP_TOPDIR"/$CHILD2/memory.min
    ...
    # @测试步骤:4: 给PARENT2分配143MB内存
    echo $$ >"$CGROUP_TOPDIR"/$PARENT2/cgroup.procs
    "$TST_TS_TOPDIR"/tst_lib/cgroup_util/bin/alloc_anon 143M &
    sleep 1
    ...
    abs_diff=${diff#-}
    assert_true [ "$(echo "$abs_diff <= $TOLERANCE" | bc)" -eq 1 ]
    assert_true [ "$aver_chidl2_memory" -le 5 ]

    # @预期结果:3: 在误差允许范围内，PARENT2分配的内存等于143MB
    TOLERANCE=$(echo "($aver_parent2_memory + 143) * 0.05" | bc)
    diff=$((aver_parent2_memory - 143))
    abs_diff=${diff#-}
    assert_true [ "$(echo "$abs_diff <= $TOLERANCE" | bc)" -eq 1 ]
