#!/bin/bash
###############################################################################
# @用例ID: 20240804-174805-982987650
# @用例名称: cgroup-v2-cpu-weight-002
# @用例级别: 1
# @用例标签: cgroup-v2 cpu weight
# @用例类型: 测试cpu.weight接口文件，测试层次结构
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
tc_setup() {
    ...
    # @预置条件: 启用cpu控制器
    echo "+cpu" >"$CGROUP_TOPDIR"/cgroup.subtree_control
...
do_test() {
    ...
    echo "+cpu" >"$CGROUP_TOPDIR"/$CHILD1/cgroup.subtree_control || return 1
    mkdir "$CGROUP_TOPDIR"/$CHILD10
    mkdir "$CGROUP_TOPDIR"/$CHILD11

    # @测试步骤:5: 对不同的cgroup设置不同的cpu.weight值
    echo 2048 >"$CGROUP_TOPDIR"/$CHILD0/cpu.weight
    echo 2048 >"$CGROUP_TOPDIR"/$CHILD1/cpu.weight
    echo 1000 >"$CGROUP_TOPDIR"/$CHILD10/cpu.weight
    echo 1000 >"$CGROUP_TOPDIR"/$CHILD11/cpu.weight

    # @测试步骤:6: 启动三个占用CPU的进程，并将其放入不同的cgroup，保证三个进程在同一个CPU核心上运行
    {
        while true; do :; done
    } &
    task_pid1=$!
    ...
    task_pid2=$!
    echo $task_pid2 >"$CGROUP_TOPDIR"/"$CHILD10"/cgroup.procs
    {
        while true; do :; done
    } &
    task_pid3=$!
    echo $task_pid3 >"$CGROUP_TOPDIR"/"$CHILD11"/cgroup.procs

    sleep 5

    # @测试步骤:7: 获取三个cgroup的CPU使用时间
    cpu_usage_usec1=$(grep usage_usec "$CGROUP_TOPDIR"/"$CHILD0"/cpu.stat | awk '{print $2}')
    cpu_usage_usec2=$(grep usage_usec "$CGROUP_TOPDIR"/"$CHILD10"/cpu.stat | awk '{print $2}')
    cpu_usage_usec3=$(grep usage_usec "$CGROUP_TOPDIR"/"$CHILD11"/cpu.stat | awk '{print $2}')
    msg "cpu usage usec1: $cpu_usage_usec1"
    msg "cpu usage usec2: $cpu_usage_usec2"
    msg "cpu usage usec3: $cpu_usage_usec3"

    # @预期结果:1: 验证CPU使用时间比例是否符合预期，即2:1:1
    usage_ratio0=$(echo "scale=2; $cpu_usage_usec1 / $cpu_usage_usec2" | bc)
    usage_ratio0=$(printf "%.0f" "$usage_ratio0")
    msg "usage_ratio0: $usage_ratio0"
    assert_true [ "$usage_ratio0" -eq 2 ]
    usage_ratio1=$(echo "scale=2; $cpu_usage_usec2 / $cpu_usage_usec3" | bc)
    msg "usage_ratio1: $usage_ratio1"
    usage_ratio1=$(printf "%.0f" "$usage_ratio1")
    msg "usage_ratio1: $usage_ratio1"
    assert_true [ "$usage_ratio1" -eq 1 ]
...
tc_teardown() {
    ...
    # @清理工作: 清理后台进程
    kill -9 "$task_pid1" "$task_pid2" "$task_pid3"
    sleep 1
    # @清理工作: 删除cgroup
    rmdir "$CGROUP_TOPDIR"/$CHILD10
    rmdir "$CGROUP_TOPDIR"/$CHILD11
