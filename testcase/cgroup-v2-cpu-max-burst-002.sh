#!/bin/bash
###############################################################################
# @用例ID: 20240804-224758-298815496
# @用例名称: cgroup-v2-cpu-max-burst-002
# @用例级别: 2
# @用例标签: cgroup-v2 cpu max burst
# @用例类型: 测试cpu.max.burst接口文件，设置限制，不设置burst时间
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-cpu-max-burst-002"
...
do_test() {
    ...
    nr_burst=$(echo "$cpu_stat" | grep nr_burst | awk '{print $2}')
    msg "nr_burst: $nr_burst"
    burst_usec=$(echo "$cpu_stat" | grep burst_usec | awk '{print $2}')
    msg "burst_usec: $burst_usec"

    # @预期结果:1: cpu.stat文件中的burst_usec和nr_bursts均等于0
    if [ "$nr_burst" -eq 0 ]; then
        assert_true [ "$burst_usec" -eq 0 ]
    else
        assert_false [ true ]
    fi
