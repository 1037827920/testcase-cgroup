#!/bin/bash
###############################################################################
# @用例ID: 20240731-141907-292150738
# @用例名称: cgropu-v1-hugetlb-limit_in_bytes-001
# @用例级别: 1
# @用例标签: cgroup-v1 hugetbl limit_in_bytes
# @用例类型: 测试hugetlb.2MB.limit_in_bytes接口文件，超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgropu-v1-hugetlb-limit_in_bytes-001"
...
do_test() {
    ...
    "${TST_TS_TOPDIR}"/tst_lib/cgroup_util/bin/alloc_hugepage 2 "$HUGEPAGE_SIZE_MB" &>/dev/null
    ...
    # @预期结果:1: max_usage_in_bytes等于2MB, failcnt大于0
