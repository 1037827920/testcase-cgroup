#!/bin/bash
###############################################################################
# @用例ID: 20240812-232817-897631780
# @用例名称: cgroup-v2-max-descendants
# @用例级别: 2
# @用例标签: cgroup-v2 max descendants
# @用例类型: 测试cgroup.max.descendants接口文件
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-max-descendants"
...
do_test() {
    ...
    # @测试步骤:4: 再创建一个子cgroup
    ret=0
    mkdir "$CGROUP_TOPDIR"/$CHILD1 || ret=-1
