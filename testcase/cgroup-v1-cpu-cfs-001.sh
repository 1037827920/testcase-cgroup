#!/bin/bash
###############################################################################
# @用例ID: 20240711-115955-308386734
# @用例名称: cgroup-v1-cpu-cfs-001
# @用例级别: 1
...
do_test() {
    ...
    echo 400000 >"$CGROUP_TOPDIR"/cpu/"${CGROUP}"/cpu.cfs_quota_us
    ...
    # @预期结果:1: cpu.stat文件中的nr_periods和nr_throttled，throttled_time均不为零
    if [ "$nr_periods" -gt 0 ] && [ "$nr_throttled" -gt 0 ]; then
        assert_true [ "$throttled_time" -gt 0 ]
    else
        assert_false [ true ]
    fi
