#!/bin/bash
###############################################################################
# @用例ID: 20240810-103238-037013530
# @用例名称: cgroup-v2-cpu-weight-nice-002
# @用例级别: 2
# @用例标签: cgroup-v2 cpu weight nice
# @用例类型: 测试cpu.weight.nice接口文件, 测试层次结构
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
do_test() {
    ...
    echo 10 >"$CGROUP_TOPDIR"/$CHILD10/cpu.weight.nice
    echo 0 >"$CGROUP_TOPDIR"/$CHILD11/cpu.weight.nice
    ...
    # @预期结果:1: 验证CPU使用时间比例是否符合预期，CHILD0小于CHILD1，CHILD10小于CHILD11
    total_cpu_usage_usec=$((cpu_usage_usec2 + cpu_usage_usec3))
    assert_true [ "$cpu_usage_usec1" -lt "$total_cpu_usage_usec" ]
    assert_true [ "$cpu_usage_usec2" -lt "$cpu_usage_usec3" ]
