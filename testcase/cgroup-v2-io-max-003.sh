#!/bin/bash
###############################################################################
# @用例ID: 20240808-110220-353546271
# @用例名称: cgroup-v2-io-max-003
# @用例级别: 1
# @用例标签: cgroup-v2 io max
# @用例类型: 测试io.max接口文件的riops
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
do_test() {
    ...
    # @测试步骤:2: 设置io.max
    echo "$device_number riops=100" >"$CGROUP_TOPDIR"/$CGROUP/io.max

    # @测试步骤:3: 启动dd进程
    echo $$ >"$CGROUP_TOPDIR"/$CGROUP/cgroup.procs
    dd iflag=direct if=$disk of=/dev/null bs=4K count=500 &>"$TMP_FILE"
    time=$(grep -oP '(?<=, )\d+\.\d+(?= s)' "$TMP_FILE")
    msg "time: $time"

    # @测试步骤:4: 获取读取速度
    deivce_line=$(grep "$device_number" "$CGROUP_TOPDIR"/$CGROUP/io.stat)
    read_iops=$(echo "$deivce_line" | grep -oP "rios=\K\d+")
    ...
    if [ "$(echo "$read_iops_per_second > $down_read_iops" | bc)" -eq 1 ]; then
        assert_true [ "$(echo "$read_iops_per_second < $up_read_iops" | bc)" -eq 1 ]
    else
        assert_false [ true ]
    fi
