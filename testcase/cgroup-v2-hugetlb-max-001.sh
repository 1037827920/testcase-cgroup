#!/bin/bash
###############################################################################
# @用例ID: 20240809-160906-120322462
# @用例名称: cgroup-v2-hugetlb-max-001
# @用例级别: 1
# @用例标签: cgroup-v2 hugetlb max
# @用例类型：测试hugetlb.2MB.max接口文件，超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-hugetlb-max-001"
...
do_test() {
    ...
    # @测试步骤:6: 获取CGROUP的hugetlb.2MB.max值
    max=$(cat "$CGROUP_TOPDIR"/$CGROUP/hugetlb.2MB.max)

    # @预期结果:1: 验证hugetlb.2MB.max的值是否为max
    assert_true [ "$max" = "max" ]

    # @测试步骤:7: 设置CGROUP的hugetlb.2MB.max为2MB
