#!/bin/bash
###############################################################################
# @用例ID: 20240801-130248-928221366
# @用例名称: cgroup-v1-hugetlb-rsvd-limit_in_bytes-002
# @用例级别: 2
# @用例标签: cgroup-v1 hugetbl rsvd limit_in_bytes
# @用例类型: 测试hugetlb.2MB.rsvd.limit_in_bytes接口文件，不超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v1-hugetlb-rsvd-limit_in_bytes-002"
...
do_test() {
    ...
    # @测试步骤:6: 获取cgroup的hugetlb.2MB.rsvd.max_usage_in_bytes和hugetlb.2MB.rsvd.failcnt
    MAX_USAGE=$(cat "$CGROUP_TOPDIR"/hugetlb/$CGROUP/hugetlb.2MB.rsvd.max_usage_in_bytes)
    ...
    if [ "$MAX_USAGE" -eq 2097152 ] && [ "$FAILCNT" -eq 0 ]; then
        assert_true [ true ]
    else
        assert_false [ true ]
    fi
