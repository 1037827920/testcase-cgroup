#!/bin/bash
###############################################################################
# @用例ID: 20240812-183828-115078858
# @用例名称: cgroup-v1-cpuacct-usage_percpu_sys
# @用例级别: 2
# @用例标签: cgroup-v1 cpuacct usage_percpu_sys
# @用例类型: 测试cpuacct.usage_percpu_sys接口文件
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v1-cpuacct-usage_percpu_sys"
...
do_test() {
    ...
    usage_percpu_sys_before=$(cat "$CGROUP_TOPDIR"/cpuacct/$CGROUP/cpuacct.usage_percpu_sys)
    usage_percpu_sys_before=$(echo "$usage_percpu_sys_before" | awk -v col="$((first_online_cpu + 1))" '{print $col}')
    ...
    # @测试步骤:6: 记录运行任务后的所有任务在每个CPU上使用内核态CPU的时间
    usage_percpu_sys_after=$(cat "$CGROUP_TOPDIR"/cpuacct/$CGROUP/cpuacct.usage_percpu_sys)
    usage_percpu_sys_after=$(echo "$usage_percpu_sys_after" | awk -v col="$((first_online_cpu + 1))" '{print $col}')
    msg "usage_percpu_sys_after: $usage_percpu_sys_after"

    # @预期结果:1: 验证统计数据是否正确反映了内核态CPU使用情况，即内核态CPU使用时间增加
    assert_true [ "$usage_percpu_sys_after" -gt "$usage_percpu_sys_before" ]
