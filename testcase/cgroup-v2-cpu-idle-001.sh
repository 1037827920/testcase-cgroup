#!/bin/bash
###############################################################################
# @用例ID: 20240805-223037-169024376
# @用例名称: cgroup-v2-cpu-idle-001
# @用例级别: 1
# @用例标签: cgroup-v2 cpu idle
# @用例类型: 测试cpu.idle接口文件，设置SCHD_IDLE调度策略
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
do_test() {
    ...
    # @测试步骤:5: 设置CHILD0的cpu.idle值为1
    echo 1 >"$CGROUP_TOPDIR"/$CHILD0/cpu.idle

    # @测试步骤:6: 启动一个占用CPU的进程，并将其放入CHILD0
    {
        while true; do :; done
    } &
    task_pid1=$!
    ...
    # @测试步骤:7: 启动另一个占用CPU的进程, 并将其放入CHILD1
    {
        while true; do :; done
    } &
    task_pid2=$!
    ...
    # @测试步骤:8: 使用top命令循环获取task_pid的cpu利用率
    total_cpu_usage=0
    ...
    # @预期结果:1: 在误差范围内，task_pid的cpu利用率应该为0
    aver_cpu_usage_max=5
    assert_true [ "$(echo "$aver_cpu_usage <= $aver_cpu_usage_max" | bc)" -eq 1 ]
