#!/bin/bash
###############################################################################
# @用例ID: 20240801-115940-264647997
# @用例名称: cgroup-v1-hugetlb-limit_in_bytes-004
# @用例级别: 2
# @用例标签: cgroup-v1 hugetbl limit_in_bytes
# @用例类型: 测试hugetlb.1GB.limit_in_bytes接口文件，不超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v1-hugetlb-limit_in_bytes-004"
...
do_test() {
    ...
    # @测试步骤:4: 设置cgroup的hugetlb.limit_in_bytes为1024MB
    echo 1024M >"$CGROUP_TOPDIR"/hugetlb/$CGROUP/hugetlb.1GB.limit_in_bytes
    ...
    # @测试步骤:6: 获取cgroup的hugetlb.1GB.max_usage_in_bytes和hugetlb.1GB.failcnt
    MAX_USAGE=$(cat "$CGROUP_TOPDIR"/hugetlb/$CGROUP/hugetlb.1GB.max_usage_in_bytes)
    msg "max usage: $MAX_USAGE"
    FAILCNT=$(cat "$CGROUP_TOPDIR"/hugetlb/$CGROUP/hugetlb.1GB.failcnt)
    msg "failcnt: $FAILCNT"

    # @预期结果:1: max_usage_in_bytes等于1GB, failcnt等于0
