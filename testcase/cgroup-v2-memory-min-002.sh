#!/bin/bash
###############################################################################
# @用例ID: 20240813-014222-140891925
# @用例名称: cgroup-v2-memory-min-002
# @用例级别: 1
# @用例标签: cgroup-v2 memory min
# @用例类型: 测试memoruy.min接口文件，parent1/2的父cgroup设置的内存上限是200MB, parent2分配的内存为170MB, 会占用parent1的memory.min保护的内存50MB，因此会被kill
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-memory-min-002"
...
do_test() {
    ...
    # @预期结果:2: 在误差允许范围内，PARENT2分配的内存等于0MB
    assert_true [ "$aver_parent2_memory" -le 5 ]

    # @预期结果:3: 检查PARENT2的oom事件次数大于0
