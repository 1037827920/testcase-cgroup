#!/bin/bash
###############################################################################
# @用例ID: 20240809-165555-512291475
# @用例名称: cgroup-v2-hugetlb-max-002
# @用例级别: 2
# @用例标签: cgroup-v2 hugetlb max
# @用例类型：测试hugetlb.2MB.max接口文件， 不超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-hugetlb-max-002"
TOLERANCE=0.3
...
do_test() {
    ...
    # @测试步骤:6: 设置CGROUP的hugetlb.2MB.max为2MB
    echo 2M >"$CGROUP_TOPDIR"/$CGROUP/hugetlb.2MB.max

    # @测试步骤:7: 分配1个hugepage
    echo $$ >"$CGROUP_TOPDIR"/$CGROUP/cgroup.procs
    "${TST_TS_TOPDIR}"/tst_lib/cgroup_util/bin/alloc_hugepage 1 "$hugepage_size_mb" &>/dev/null &

    # @测试步骤:8: 获取hugetlb.2MB.current
    total_hugepage_current=0
    for _ in $(seq 1 5); do
        current=$(cat "$CGROUP_TOPDIR"/$CGROUP/hugetlb.2MB.current)
