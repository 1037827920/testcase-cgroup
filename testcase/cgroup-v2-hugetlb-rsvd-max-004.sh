#!/bin/bash
###############################################################################
# @用例ID: 20240809-185417-880857496
# @用例名称: cgroup-v2-hugetlb-rsvd-max-004
# @用例级别: 2
# @用例标签: cgroup v2 hugetlb rsvd max
# @用例类型：测试hugetlb.1GB.rsvd.max接口文件， 不超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-hugetlb-rsvd-max-004"
TOLERANCE=0.3
...
do_test() {
    ...
    echo 1024M >"$CGROUP_TOPDIR"/$CGROUP/hugetlb.1GB.rsvd.max

    # @测试步骤:7: 分配1个hugepage
    echo $$ >"$CGROUP_TOPDIR"/$CGROUP/cgroup.procs
    "${TST_TS_TOPDIR}"/tst_lib/cgroup_util/bin/alloc_hugepage 1 "$hugepage_size_mb" &>/dev/null &

    # @测试步骤:8: 获取hugetlb.1GB.rsvd.current
    total_hugepage_current=0
    for _ in $(seq 1 5); do
        current=$(cat "$CGROUP_TOPDIR"/$CGROUP/hugetlb.1GB.rsvd.current)
    ...
    # @预期结果:1: 在误差允许的范围内，aver_hugepage_current等于1024
    diff=$((aver_hugepage_current - 1024))
    abs_diff=${diff#-}
    assert_true [ "$(echo "$abs_diff < $TOLERANCE" | bc)" -eq 1 ]

    # @测试步骤:9: 获取hugetlb.1GB.events的max字段
    max_cnt=$(grep "max" "$CGROUP_TOPDIR"/$CGROUP/hugetlb.1GB.events | awk '{print $2}')
