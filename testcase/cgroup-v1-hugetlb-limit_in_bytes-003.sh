#!/bin/bash
###############################################################################
# @用例ID: 20240801-115938-131086948
# @用例名称: cgroup-v1-hugetlb-limit_in_bytes-003
# @用例级别: 1
# @用例标签: cgroup-v1 hugetbl limit_in_bytes
# @用例类型: 测试hugetlb.1GB.limit_in_bytes接口文件，超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v1-hugetlb-limit_in_bytes-003"
...
do_test() {
    ...
    # @测试步骤:4: 设置cgroup的hugetlb.limit_in_bytes为512MB
    echo 512M >"$CGROUP_TOPDIR"/hugetlb/$CGROUP/hugetlb.1GB.limit_in_bytes
    ...
    # @测试步骤:6: 获取cgroup的hugetlb.1GB.failcnt
