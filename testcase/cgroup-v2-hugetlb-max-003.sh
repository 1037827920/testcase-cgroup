#!/bin/bash
###############################################################################
# @用例ID: 20240809-171017-765935943
# @用例名称: cgroup-v2-hugetlb-max-003
# @用例级别: 1
# @用例标签: cgroup v2 hugetlb max
# @用例类型：测试hugetlb.1GB.max接口文件， 超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v2-hugetlb-max-003"
...
do_test() {
    ...
    # @测试步骤:6: 获取CGROUP的hugetlb.1GB.max值
    max=$(cat "$CGROUP_TOPDIR"/$CGROUP/hugetlb.1GB.max)

    # @预期结果:1: 验证hugetlb.1GB.max的值是否为max
    assert_true [ "$max" = "max" ]

    # @测试步骤:7: 设置CGROUP的hugetlb.1GB.max为512MB
    echo 512M >"$CGROUP_TOPDIR"/$CGROUP/hugetlb.1GB.max
