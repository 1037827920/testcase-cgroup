#!/bin/bash
###############################################################################
# @用例ID: 20240721-163338-552003922
# @用例名称: cgroup-v1-blkio-bfq-weight_device
# @用例级别: 1
# @用例标签: cgroup-v1 blkio bfq-weight_device
# @用例类型: 测试接口文件blkio.bfq.weight_device
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v1-blkio-bfq-weight_device"
...
do_test() {
    ...
    # @测试步骤:5: 设置CHILD0\CHILD1\CHILD2的blkio.bfq.weight_device
    echo "$disk_device 100" >"$CGROUP_TOPDIR"/blkio/$CHILD0/blkio.bfq.weight_device
    echo "$disk_device 200" >"$CGROUP_TOPDIR"/blkio/$CHILD1/blkio.bfq.weight_device
    echo "$disk_device 300" >"$CGROUP_TOPDIR"/blkio/$CHILD2/blkio.bfq.weight_device
