#!/bin/bash
###############################################################################
# @用例ID: 20240719-150059-269990543
# @用例名称: cgroup-v1-cpuset-memory_spread_page-002
# @用例级别: 2
# @用例标签: cgroup-v1 cpuset memory_spread_page
# @用例类型: 测试cpuset.memory_spread_page接口文件，测试将page cache平均分布道各个节点中
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v1-cpuset-memory_spread_page-002"
...
do_test() {
    ...
    # @测试步骤:6: 启用cpuset.memory_spread_page
    echo 1 >"$CGROUP_TOPDIR"/cpuset/$CGROUP/cpuset.memory_spread_page

    # @测试步骤:7: 获取numa node0和numa node1的FilePages
    node0_file_pages=$(numastat -m | grep FilePages | awk '{print $2}')
    node1_file_pages=$(numastat -m | grep FilePages | awk '{print $3}')
    msg "node0_file_pages: $node0_file_pages"
    msg "node1_file_pages: $node1_file_pages"

    # @测试步骤:8: 运行一个需要申请page cache的程序
    "$TST_TS_TOPDIR"/tst_lib/cgroup_util/bin/alloc_pagecache 100M &
    task_pid=$!
    msg "task_pid: $task_pid"
    echo $task_pid >"$CGROUP_TOPDIR"/cpuset/$CGROUP/cgroup.procs
    sleep 2

    # @测试步骤:9: 不断检查numa节点的FilePages大小
    ...
    for _ in $(seq 1 8); do
        ...
        msg "node0_current_pagecache: $node0_current_pagecache"
        msg "node1_current_pagecache: $node1_current_pagecache"
    ...
    # @预期结果:1: 在误差允许范围内，node0_diff = 50, node1_diff = 50
    num0_diff=$(echo "scale=2; $aver_node0_pagecache - $node0_file_pages" | bc)
    abs_num0_diff=${num0_diff//-/}
    num1_diff=$(echo "scale=2; $aver_node1_pagecache - $node1_file_pages" | bc)
    abs_num1_diff=${num1_diff//-/}
    if [ "$(echo "$abs_num0_diff > 48 " | bc)" ] && [ "$(echo "$abs_num0_diff < 52" | bc)" ]; then
        assert_true [ true ]
    else
        assert_true [ false ]
    fi
    if [ "$(echo "$abs_num1_diff > 48" | bc)" ] && [ "$(echo "$abs_num1_diff < 52" | bc)" ]; then
        assert_true [ true ]
    else
        assert_true [ false ]
    fi
