#!/bin/bash
###############################################################################
# @用例ID: 20240801-130249-962716187
# @用例名称: cgroup-v1-hugetlb-rsvd-limit_in_bytes-003
# @用例级别: 1
# @用例标签: cgroup-v1 hugetbl rsvd limit_in_bytes
# @用例类型: 测试hugetlb.1GB.rsvd.limit_in_bytes接口文件，超过限制
###############################################################################
[ -z "$TST_TS_TOPDIR" ] && {
...
CGROUP="cgroup-v1-hugetlb-rsvd-limit_in_bytes-003"
...
do_test() {
    ...
    echo 512M >"$CGROUP_TOPDIR"/hugetlb/$CGROUP/hugetlb.1GB.rsvd.limit_in_bytes
